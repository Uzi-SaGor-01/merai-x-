...
exec("rm -rf script/commands/cache && mkdir -p script/commands/cache && rm -rf script/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("SAGOR ISLAM ❯ ") + chalk.hex("#00ffff")("fixed by Sagor islam"))
});
...
try {
    ...
    logger.loader("Found file config: Sagor.json");
}
catch {
    ...
    else return logger.loader("Sagor.json not found!", "error");
}
...
catch { return logger.loader("Can't load file Sagor!", "error") }
...
console.log(global.getText('mirai', 'foundPathSagorstate'))
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "Sagorstate.json"));
    var appState = require(appStateFile);
    logger.loader(global.getText("mirai", "foundPathSagorstate"))
}
catch { return logger.loader(global.getText("mirai", "notFoundPathSagorstate"), "error") }
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Sagor`, '[ Sagor (Islam) ]');
        logger('Hey, thank you for using this Bot', '[ Sagor (islam) ]');
        logger("Fixed By Sagor", '[ Sagor (Islam) ]');
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '[ SAGOR-DATABASE ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ SAGOR-DATABASE ]'); }
